...
void main(){      
  ...
  //initialise double sized buffer to zero
   for ( i = 0; i< 2*N; i++)
  { 
  	extend[i] = 0.0;
  }
...
void lab4(void)
{
	...
	//mono_write_16Bit(cir_op_2());//original circular 
}
...
//--------------------------------------------------------------------------------
double cir_op_1 (void)//original circular 
{
	int i;
	double y = 0.0;
	x[ptr] = mono_read_16Bit();
...
//-----------------------------------------------------------------------------------------------
double cir_op_2 (void)//original circular 
{
	int i;
	double y = 0.0;
	x[ptr] = mono_read_16Bit();
	...
	for( i = 0; i < (N-1)/2; i++) // true for both odd and even number of samples
	{
		y += b[i]* ( extend[(ptr+i)%N ] + extend[ (N+ptr-1) - i] );
	} // don't need to add anything else for even
